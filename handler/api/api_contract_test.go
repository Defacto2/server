...
func TestAnnouncementsContract(t *testing.T) {
	...
	resp, err := clientDo(t, apiv1+"/category/announcements")
	be.Equal(t, err, nil)
	defer func() {
...
func TestCategoriesContract(t *testing.T) {
	...
	resp, err := clientDo(t, apiv1+"/categories")
	be.Equal(t, err, nil)
	defer func() {
		if err := resp.Body.Close(); err != nil {
			t.Log(err)
	...
	be.True(t, strings.HasPrefix(result[0].URLs.API, api.APIBase+"/artifacts/"))
...
func TestPlatformsContract(t *testing.T) {
	...
	resp, err := clientDo(t, apiv1+"/platforms")
	be.Equal(t, err, nil)
	defer func() {
		if err := resp.Body.Close(); err != nil {
			t.Log(err)
...
func TestScenersContract(t *testing.T) {
	...
	resp, err := clientDo(t, apiv1+"/sceners")
	be.Equal(t, err, nil)
	defer func() {
		if err := resp.Body.Close(); err != nil {
			t.Log(err)
	...
	for _, scener := range result.Sceners {
		...
		be.True(t, strings.HasPrefix(scener.URLs.API, api.APIBase+"/scener/"))
...
func TestScenerDetailsContract(t *testing.T) {
	...
	resp, err := clientDo(t, apiv1+"/scener/006")
	be.Equal(t, err, nil)
	defer func() {
		if err := resp.Body.Close(); err != nil {
			t.Log(err)
	...
	be.True(t, ok && strings.HasPrefix(apiURL, api.APIBase+"/scener/"))
...
func TestScenerNotFound(t *testing.T) {
	...
	url := apiv1 + "/scener/nonexistent-scener-12345"
	resp, err := clientDo(t, url)
	be.Equal(t, err, nil)
	defer func() {
		if err := resp.Body.Close(); err != nil {
...
func TestArtifactNotFound(t *testing.T) {
	...
	url := apiv1 + "/artifact/nonexistent"
	resp, err := clientDo(t, url)
	be.Equal(t, err, nil)
	defer func() {
		if err := resp.Body.Close(); err != nil {
