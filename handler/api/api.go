...
// NOTE: This api package was mostly generated by Minstal's devstral-2 model,
...
var (
	apiCache   = make(map[string]cacheQueryItem)
...
var (
	reMarkup1 = regexp.MustCompile(`<a\b[^>]*>([^<]*)<\/a>`)
	reMarkup2 = regexp.MustCompile(`<a\s+([^>]*href="([^"]*)"[^>]*)>([^<]*)<\/a>`)
	reMarkup3 = regexp.MustCompile(`(class|style|id|title|data-[a-z-]*)="[^"]*"`)
	reMarkup4 = regexp.MustCompile(`<\s*([a-z0-9]+)\s+>`)
	reMarkup5 = regexp.MustCompile(`>\s+<`)
	reMarkup6 = regexp.MustCompile(`[\t\r\n]+`)
	reMarkup7 = regexp.MustCompile(`\s{2,}`)
)

// Markup removes CSS classes and attributes from HTML for API responses.
// Keeps semantic HTML tags but removes presentation-specific markup.
func Markup(src string) string {
	if src == "" {
		return src
	}
	...
	src = reMarkup1.ReplaceAllStringFunc(src, func(match string) string {
		if strings.Contains(match, `href="`) {
			return match // Keep anchors with href
		}
		// Extract content from anchors without href
		if matches := reMarkup1.FindStringSubmatch(match); len(matches) > 1 {
			return matches[1]
		}
		return match
	})
	...
	src = reMarkup5.ReplaceAllString(src, "><")

	// Clean up multiple spaces (but preserve line breaks for readability)
	src = reMarkup6.ReplaceAllString(src, " ")
	src = reMarkup7.ReplaceAllString(src, " ")
...
func milestonesAPI(c *echo.Context, highlights bool) error {
	...
	all := app.Collection()
	result := make(app.Milestones, len(all))
	for i, m := range all {
	...
	if highlights {
		...
		result = slices.DeleteFunc(result, func(m app.Milestone) bool {
			return !m.Highlight
		})
	}
...
func MilestoneYearAPI(c *echo.Context) error {
	...
	var result app.Milestones
	for _, m := range app.Collection() {
		if m.Year == year {
...
func milestoneFmt(m app.Milestone) app.Milestone {
	m.ContentHTML = Markup(m.Content)
	m.Content = simple.CleanHTML(m.Content)
	m.Lead = Markup(m.Lead)
	if len(m.List) > 0 {
		for i := range m.List {
			m.List[i].LinkTitle = Markup(m.List[i].LinkTitle)
			m.List[i].SubTitle = Markup(m.List[i].SubTitle)
		}
	}
...
func scenersAPI(s model.Sceners) []scenerAPI {
	...
	for _, s := range sceners {
		...
		name := helper.Slug(s)
		html, _ := app.LinkScnr(s) // any errors can be ignored as it will leave "HTML" blank
		// create stable ID from the url using an obfuscated name
		id := simple.Hash(name)
