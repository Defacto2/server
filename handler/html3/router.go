...
// Route for the /html3 sub-route group.
// Any errors are logged and rendered to the client using HTTP codes
// and the custom /html3, group error template.
func Route(sl *slog.Logger, e *echo.Echo, db *sql.DB) *echo.Group {
	if err := nils.Check(db, e); err != nil {
		panic(fmt.Errorf("route html3: %w", err))
	}

	g := e.Group(Prefix)
	g.GET("", func(c *echo.Context) error {
		return Index(sl, c, db)
	})
...
func custom404(g *echo.Group) *echo.Group {
	g.GET("/:uri", func(c *echo.Context) error {
