...
// getFilesByCategoryHelper is a helper function that transforms database records to API format.
func getFilesByCategoryHelper(records []*models.File) []AnnouncementFile {
	...
	for i, record := range records {
		...
		// Construct the file record for description
...
// CategoryAPI returns a list of files from any category.
func CategoryAPI(c echo.Context, db *sql.DB, sl *slog.Logger) error {
	const msg = "get files by category"
	if err := panics.EchoContextDS(c, db, sl); err != nil {
		return fmt.Errorf("%s: %w", msg, err)
	}

	// Get category parameter from URL
	categoryURI := c.Param("category")
	if categoryURI == "" {
	...
	// Query files using the html3 query functions directly
	ctx := context.Background()
	...
	if limitStr != "" {
		...
		// Cap the limit at Maximum to prevent abuse
		if limit > model.Maximum {
			limit = model.Maximum
		}
	}

	order := html3.Clauses(c.QueryString())

	// Declare variables first
	var records models.FileSlice
	var err error

	// Check if the URI is a platform first
	// Get all platform URIs (from FirstPlatform to LastPlatform)
	allURIs := tags.URIs()
	isPlatform := false
	for tag := tags.FirstPlatform; tag <= tags.LastPlatform; tag++ {
		if allURIs[tag] == categoryURI {
			isPlatform = true
			break
		}
	}
	...
	if isPlatform {
		sl.Info(msg, slog.String("platform query attempt", categoryURI))
		records, err = order.ByPlatform(ctx, db, page, limit, categoryURI)
		if err != nil {
			sl.Error(msg, slog.String("query", "platform files"), slog.String("platform", categoryURI), slog.Any("error", err))
			return c.JSON(http.StatusInternalServerError, map[string]string{
				"error": "Failed to query platform files",
			})
		}
		sl.Info(msg, slog.String("platform query success", categoryURI), slog.Int("records", len(records)))
	} else {
		// Try category query for non-platform URIs
		records, err = order.ByCategory(ctx, db, page, limit, categoryURI)
		if err != nil {
			sl.Error(msg, slog.String("query", "category files"), slog.String("category", categoryURI), slog.Any("error", err))
			return c.JSON(http.StatusInternalServerError, map[string]string{
	...
	// Try to count by category first, fall back to platform count
	count, err := model.CategoryCount(ctx, db, categoryURI)
	if err != nil {
		// If category count fails, try platform count
		// Convert URI to platform tag name for database query
		platformName := tags.NameByURI(categoryURI)
		count, err = model.PlatformCount(ctx, db, platformName)
		if err != nil {
			sl.Error(msg, slog.String("query", "platform count"), slog.String("platform", platformName), slog.Any("error", err))
			return c.JSON(http.StatusInternalServerError, map[string]string{
	...
	// Try to sum by category first, fall back to platform sum
	byteSum, err := model.CategoryByteSum(ctx, db, categoryURI)
	if err != nil {
		// If category byte sum fails, try platform byte sum
		// Convert URI to platform tag name for database query
		platformName := tags.NameByURI(categoryURI)
		byteSum, err = model.PlatformByteSum(ctx, db, platformName)
		if err != nil {
			sl.Error(msg, slog.String("query", "platform byte sum"), slog.String("platform", platformName), slog.Any("error", err))
			return c.JSON(http.StatusInternalServerError, map[string]string{
	...
	files := getFilesByCategoryHelper(records)
	...
	response := struct {
		...
		Pagination: struct {
			...
			CurrentPage: page,
			TotalPages:  helper.PageCount(countInt, limit),
			TotalFiles:  countInt,
