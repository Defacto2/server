...
var areaCodes = sync.OnceValue(func() []NAN {
...
var validAreaCodes = sync.OnceValue(func() map[NAN]struct{} {
	codes := AreaCodes()
	set := make(map[NAN]struct{}, len(codes))
...
var regions = sync.OnceValue(func() []Region {
	misc := miscellaneous
	can := canadaProvinces
	usa := usaStates

	r := make([]Region, 0, len(misc)+len(can)+len(usa))
	r = append(r, misc[:]...)
	r = append(r, can[:]...)
	r = append(r, usa[:]...)

	slices.SortFunc(r, func(i, j Region) int {
...
var abbrevs = sync.OnceValue(func() []Abbreviation {
	regions := regions()
	abbr := make([]Abbreviation, 0, len(regions))
	for _, r := range regions {
		if r.Abbreviation != "" {
			abbr = append(abbr, r.Abbreviation)
		}
	}

	slices.Sort(abbr)
	return slices.Compact(abbr)
})

var regionCodes = sync.OnceValue(func() [1000][]Region {
	var table [1000][]Region
	all := regions()

	for _, r := range all {
		for _, ac := range r.AreaCodes {
			if ac >= 200 && ac <= 999 {
				table[ac] = append(table[ac], r)
			}
		}
	}
	return table
...
var regionMap = sync.OnceValue(func() map[Abbreviation]Region {
	all := regions()
	m := make(map[Abbreviation]Region, len(all))
	for _, r := range all {
		if r.Abbreviation != "" {
			// Normalize key to uppercase for case-insensitive matching
			upper := Abbreviation(strings.ToUpper(string(r.Abbreviation)))
			m[upper] = r
		}
	}
...
var regionIndex = sync.OnceValue(func() []reg {
	all := regions()
	index := make([]reg, len(all))
	for i, r := range all {
		index[i] = reg{
...
func (c NAN) HTML() template.HTML {
	...
	for i, region := range regions {
		...
		if i == first {
			html.WriteString(strconv.Itoa(int(c)))
			html.WriteString(" - ")
			html.WriteString(region.Name)
			if len(abbr) > 0 {
				html.WriteString(" (")
				html.WriteString(string(abbr))
				html.WriteString(")")
			}
			continue
		}
...
// Copy returns a copy of the North American Numbering Plan area codes map that provide additional information,
// such as when an area code was split or created and the regions it serves.
func Copy() map[NAN]string {
	return notes
}
...
// CanadaProvinces is a list of Canadian provinces in the North American Numbering Plan.
// These can be checked against official lists to ensure accuracy.
func CanadaProvinces() [13]Region {
	return canadaProvinces
}

var canadaProvinces = [...]Region{
	{"Alberta", "AB", []NAN{403}},
...
// These can be checked against official lists to ensure accuracy.
func USAStates() [51]Region {
	return usaStates
}

var usaStates = [...]Region{
	{"Alabama", "AL", []NAN{205}},
	{"Alaska", "AK", []NAN{907}},
	{"Arizona", "AZ", []NAN{602}},
	{"Arkansas", "AR", []NAN{501}},
...
// Miscellaneous regions in the North American Numbering Plan.
// These can be checked against official lists to ensure accuracy.
func Miscellaneous() [2]Region {
	return miscellaneous
}

var miscellaneous = [...]Region{
...
func Lookup(a any) []Region {
	...
	case string:
		...
		case 0, 1:
			return nil
		case abbreviation:
			r, ok := RegionByAbbr(Abbreviation(v))
			if !ok {
				return nil
			}
			return []Region{r}
...
func Query(a any) Result {
	...
	case int, uint:
		...
		if c, ok := a.(uint); ok {
			...
			return Result{Region: nil, AreaCode: NAN(int(c))}
		}
		return Result{Region: nil, AreaCode: 0}
	case NAN:
...
func Contains(target Region, regions []Region) bool {
	return slices.ContainsFunc(regions, func(r Region) bool {
		return r.Abbreviation == target.Abbreviation && r.Name == target.Name
	})
}
...
func Abbreviations() []Abbreviation {
	return slices.Clone(abbrevs())
}

// AreaCodes returns a list of all NANP area codes sorted in ascending order.
func AreaCodes() []NAN {
	return slices.Clone(areaCodes())
}

// RegionByAbbr returns the region with the given two-letter abbreviation.
func RegionByAbbr(abbr Abbreviation) (Region, bool) {
	if len(abbr) == 0 {
		return Region{}, false
	}

	upper := Abbreviation(strings.ToUpper(string(abbr)))
	r, found := regionMap()[upper]
	return r, found
}
...
func RegionByCode(code NAN) []Region {
	if !code.Valid() || code < 200 || code > 999 {
		return nil
	}

	matches := regionCodes()[code]
	if len(matches) == 0 {
		return nil
	}

	finds := make([]Region, len(matches))
	copy(finds, matches)
	return finds
}
...
func RegionByName(name string) Region {
	for region := range slices.Values(Regions()) {
...
func RegionContains(s string) []Region {
	...
	const size = 4
	matches := make([]Region, 0, size)

	index := regionIndex()

	for _, entry := range index {
		if strings.Contains(entry.lowerName, substr) {
			matches = append(matches, entry.region)
		}
	}
