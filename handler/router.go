...
func (c *Configuration) api(e *echo.Echo, db *sql.DB, sl *slog.Logger, public embed.FS) *echo.Echo {
	...
	apiGroup.GET("/files/new", func(c echo.Context) error { return app.ArtifactsNewAPI(c, db, sl) })
	apiGroup.GET("/file/:hash", func(c echo.Context) error { return app.FileAPI(c, db, sl) })
