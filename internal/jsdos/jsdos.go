...
// Cpu is the [cpu] section of the configuration file.
type Cpu struct {
	Core   Core   `ini:"core"`
...
const (
	...
	SVGA_S3       Platform = "svga_s3"       // SVGA_S3 is SuperVGA using the s3 Trio 64 chip.
	SVGA_ET3000   Platform = "svga_et3000"   // SVGA_ET3000 is SuperVGA using the Tseng Labs ET3000 graphics mode.
	SVGA_ET4000   Platform = "svga_et4000"   // SVGA_ET4000 is SuperVGA using the Tseng Labs ET4000 graphics mode.
	SVGA_Paradise Platform = "svga_paradise" // SVGA_Paradise is SuperVGA using the Paradise PVGA1A chip, common in the late 1980s.
	VESA_NoLFB    Platform = "vesa_nolfb"    // VESA_NoLFB is the VESA graphics mode without the Linear Frame Buffer that is sometimes faster than SVGA_S3.
	VESA_OldVBE   Platform = "vesa_oldvbe"   // VESA_OldVBE is the VESA graphics mode using the old VBE 1.2 standard.
)
...
func (j *Jsdos) CPU(value string) {
	...
	case "8086":
		j.Cpu.Model = I386Slow
		j.Cpu.Core = Simple
		j.Cpu.Cycles = Fix5Mhz
	case "386":
		j.Cpu.Model = I386Slow
		j.Cpu.Core = Normal
	case "486":
...
func (j *Jsdos) Machine(value string) {
	...
	case "tandy":
		...
		j.Tandy()
	case "svga":
		j.Dosbox.Machine = SVGA_S3
		j.Dosbox.MemSize = Mem486
	case "paradise":
		j.Dosbox.Machine = SVGA_Paradise
		j.Dosbox.MemSize = Mem486
	case "oldvbe":
		j.Dosbox.Machine = VESA_OldVBE
		j.Dosbox.MemSize = Mem486
	case "nolfb":
		j.Dosbox.Machine = VESA_NoLFB
		j.Dosbox.MemSize = Mem486
	case "et4000":
		j.Dosbox.Machine = SVGA_ET4000
		j.Dosbox.MemSize = Mem486
	case "et3000":
		j.Dosbox.Machine = SVGA_ET3000
		j.Dosbox.MemSize = Mem486
	case "ega":
...
func (j *Jsdos) Beeper() {
	j.Speaker.Enable = "true"
	j.Speaker.Rate = AudioRate
	j.Speaker.Tandy = "off"
	j.Speaker.Disney = "false"
}
