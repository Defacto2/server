...
var GroupnameWhere = struct {
	...
	ID:          whereHelperint{field: "\"groupnames\".\"id\""},
...
func FindGroupname(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*Groupname, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"groupnames\" where \"id\"=$1", sel,
	)
...
func (o *Groupname) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			...
			cache.query = "INSERT INTO \"groupnames\" %sDEFAULT VALUES%s"
		}
...
func (o *Groupname) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE \"groupnames\" SET %s WHERE %s",
			...
			strmangle.WhereClause("\"", "\"", len(wl)+1, groupnamePrimaryKeyColumns),
		)
...
func (o GroupnameSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"groupnames\" SET %s WHERE %s",
		...
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), len(colNames)+1, groupnamePrimaryKeyColumns, len(o)))
...
func (o *Groupname) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		if len(conflict) == 0 {
			conflict = make([]string, len(groupnamePrimaryKeyColumns))
			copy(conflict, groupnamePrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"groupnames\"", updateOnConflict, ret, update, conflict, insert)
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert groupnames")
	}
...
func (o *Groupname) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"groupnames\" WHERE \"id\"=$1"
...
func (o GroupnameSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"groupnames\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, groupnamePrimaryKeyColumns, len(o))
...
func GroupnameExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"groupnames\" where \"id\"=$1 limit 1)"
